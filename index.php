...
if ($connection == false) {
    ...
    if (isset($_GET["project"]) && !empty($_GET["project"])) {
        $tasks_by_category_sql ="SELECT t.name AS task, t.date_term AS task_date, t.file, t.status, p.name AS category FROM tasks t JOIN projects p ON p.id = t.project_id WHERE t.user_id = 1 AND p.id = " . $_GET["project"] . "";
